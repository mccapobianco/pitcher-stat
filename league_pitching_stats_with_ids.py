#code copied and pasted from pybaseball github with one adjustment (line 61)
...
def get_table(soup):
    ...
    for row in rows:
        ...
        #MY CHANGES
        if cols: #line added
       		link = cols[0]['data-append-csv'] #line added
       	mlb_ID = int(link.split('mlb_ID=')[1]) #line added
        cols = [ele.text.strip() for ele in cols]
        cols.append(mlb_ID) #line added
        data.append([ele for ele in cols])
    data[0].append('mlb_ID') #line added
    #END MY CHANGES
